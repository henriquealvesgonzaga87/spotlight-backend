...
@router.get("/location/city/{country_name}/{state_name}", status_code=status.HTTP_200_OK, response_model=list[CitySchema])
@inject
def get_cities(country_name: str, state_name: str, location_use_case: LocationUseCases = Depends(Provide[Container.location_use_cases])):
